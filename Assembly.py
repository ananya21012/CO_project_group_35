...
def mov(arr,register,count):
    ...
    if arr[2][0]=="$":
        ...
        if arr[1] not in register:
            print(arr)
            raise SyntaxError(f'INVALID REGISTER CODE , at line {count}')
        bin_string = bin_string + register[arr[1]]
        bin_string = bin_string + bin(int(arr[2][1:]))
        return bin_string

    else:
        bin_string = "1001100000"
        if arr[1] not in register and arr[2] not in register:
            raise SyntaxError(f'INVALID REGISTER CODE , at line {count}')
        elif arr[1]=="FLAGS" and arr[2]=="FLAGS":
            raise SyntaxError(f'INVALID USE OF FLAGS , at line {count}')
